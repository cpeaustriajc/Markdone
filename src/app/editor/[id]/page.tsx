...
const LegacyEditor = dynamic(
	() => import("@/components/editor/legacy").then((mod) => mod.LegacyEditor),
	{ ssr: false },
);
...
export default function Editor() {
	...
	useEffect(() => {
		const sessionStorageDrafts = JSON.parse(
			sessionStorage.getItem("markdone:drafts") as string,
		) as Draft[];
		const sessionStorageDraft =
			sessionStorageDrafts.find((draft) => draft.id === id) ?? {};
